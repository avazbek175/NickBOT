...
from src.services.text_styler import TextStyler, FONT_NAMES
...
def _cache_styles(text: str, styles: List[str], category: str) -> str:
...
def build_copy_kb_styler(token: str, style_idx: int, lang: Language) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
...
async def styler_start(message: Message, session: AsyncSession):
...
async def styler_new_text(call: CallbackQuery):
    lang = Language.UZBEK
...
@router.message(F.text.len() >= 2, ~F.text.startswith("/"), ~F.text.startswith("🏠"),
                ~F.text.startswith("✨"), ~F.text.startswith("🎮"), ~F.text.startswith("❤️"),
                ~F.text.startswith("📜"), ~F.text.startswith("🔍"), ~F.text.startswith("🎲"),
                ~F.text.startswith("👤"), ~F.text.startswith("🌐"), ~F.text.startswith("ℹ"))
async def styler_receive_text(message: Message, session: AsyncSession):
...
async def styler_category_selected(call: CallbackQuery, session: AsyncSession):
    ...
    if "<code>" in msg_text:
        import re
        m = re.search(r"<code>([^<]+)</code>", msg_text)
        if m:
            orig_text = m.group(1)
    ...
    lines.append("└───────────────────")
    lines.append(f"📂 {cat.title()} — {len(styles)} styles")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
...
def _generate_styles(text: str, category: str) -> List[str]:
    if category == "fancy":
        ...
        results = []
        for _ in range(10):
            results.append(TextStyler.invisible(text))
        return results
        ...
        results = []
        for _ in range(20):
            results.append(TextStyler.hacker_style(text))
        return results
...
async def styler_page(call: CallbackQuery, session: AsyncSession):
    parts = call.data.split(":")
    if len(parts) < 3:
        await call.answer()
        return
...
@router.callback_query(F.data.startswith("stcp:"))
async def styler_copy(call: CallbackQuery, session: AsyncSession):
    parts = call.data.split(":")
    if len(parts) < 3:
        await call.answer()
        return
    ...
    kb = build_copy_kb_styler(token, idx, lang)
...
async def styler_back(call: CallbackQuery, session: AsyncSession):
    parts = call.data.split(":")
    if len(parts) < 2:
        await call.answer()
        return
